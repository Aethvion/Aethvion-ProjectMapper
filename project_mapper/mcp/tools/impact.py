...
SCHEMA = {
    ...
    "inputSchema": {
        "type": "object",
        "properties": {
            "entity": {"type": "string", "description": "Name or ID of the entity to analyse."},
            "depth": {
                "type": "integer",
...
def handle_pm_impact(args: dict[str, Any], ctx: MCPContext) -> str:
    ...
    subject = (
        subject_raw.get("name", entity_name) if isinstance(subject_raw, dict) else str(subject_raw)
    )
    ...
    lines.append("If you change or delete this entity, review all listed dependents.")
    return "\n".join(lines).rstrip()
